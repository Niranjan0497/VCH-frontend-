...
const DoctorList = ({ doctors = [], gender }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row mx-6">
      {/* Doctor List Section */}
      <div className="lg:w-2/3 h-auto lg:h-[700px] overflow-y-scroll scrollbar-hide mb-4 lg:mb-0">
        {filteredDoctors.length ? (
          filteredDoctors.map((doctor) => (
            <div
              key={doctor.id}
              className="p-4 mb-4 bg-white rounded-lg shadow-md cursor-pointer"
              onClick={() => setSelectedLocation(doctor.mapLocation)}
            >
              <img
                src={doctor.image}
                alt={doctor.name}
                className="w-20 h-20 rounded-full border float-left mr-4"
              />
              <div>
                <NavLink to={`/onedoctor/${doctor.id}`}>
                  <h2 className="text-xl font-semibold text-blue-900">{doctor.name}</h2>
                ...
                <p className="text-gray-700">{doctor.specialization}</p>
                <NavLink to={`/onedoctor/${doctor.id}`}>
                  <button className="bg-blue-600 text-white px-3 py-2 rounded-lg mr-2 float-right">
                    Book Clinic Visit
                  </button>
                </NavLink>
                <p className="text-gray-500">{doctor.experience} experience</p>
                <p className="font-semibold flex items-center text-gray-800">
                  <MapPin className="w-4 h-4 mr-1 text-gray-600" /> {doctor.location} • {doctor.clinic}
                </p>
                <button
                  className="flex border border-blue-600 text-blue-600 px-3 py-2 rounded-lg float-right"
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleContact(doctor.id);
                  }}
                >
              ...
              {contactVisibility[doctor.id] && (
                <div className="mt-4 clear-both">
                  ...
                  <p className="text-xs text-gray-500">
                    By calling this number, you agree to the Terms & Conditions. If you could not connect with the center, please write to support@practo.com
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="lg:w-1/3">
        ...
        {selectedLocation && (
          <iframe
            ...
            className="w-full h-[300px] lg:h-[500px] rounded-lg shadow-md"
